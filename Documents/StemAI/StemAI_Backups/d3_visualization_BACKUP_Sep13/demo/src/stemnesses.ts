...
export function setupStemnesses(element: string) {

    const full_data = graph.stemnesses
    ...
    // Create graph area
    const graphGenerator = svg.selectAll(".stemnessesGroup")
        .data([0])
        .join("g")
    ...
    line_generators.forEach((lineGenerator, index) => {
        graphGenerator
              .selectAll(`.stemnessesTimeline-${index}`)
              .data([Object.values(full_data)[index]])
              .join("path")
              .attr("class", `stemnessesTimeline-${index}`)
    ...
    line_generators.forEach((lineGenerator, index) => {
            graphGenerator
                  .selectAll(`.Stemnesses-${index}`)
                  .data([Object.values(full_data)[index].slice(0,1)])
                  .join("path")
                  .attr("class", `Stemnesses-${index}`)
                  .attr("d", d => lineGenerator(d))
                  .attr("stroke", shuffledColorArray[index])
                  .attr("fill", "none")
                  .attr("opacity", 1);
...
export function animateStemnesses(element: string, timestamp: number) {
    const full_data = graph.stemnesses
